...
public final class Koorde extends LinearWalker {
	...
	private synchronized void startEdgeFixer() {
		...
		if (config.getUseTimerInsteadOfThread()) {
			Timer.getSingletonTimer().schedule(this.edgeFixer,
...
*/
	...
	public String getRoutingTableString() {
		...
		sb.append(super.getRoutingTableString());	// successors and predecessor
		sb.append("\n");
	...
	private class EdgeFixer implements Runnable {
		...
		private void sleep(boolean backupsToBeUpdated) throws InterruptedException {
			if (backupsToBeUpdated) {
				this.interval = config.getFixEdgeMinInterval();
			}
			else {
				this.interval += config.getFixEdgeIntervalDelta();
