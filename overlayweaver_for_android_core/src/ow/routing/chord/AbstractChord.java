...
//System.out.println("On " + selfIDAddress.getAddress() + ", closestNodes(): " + target);
		...
		// init finger table index
		BigInteger distance = this.distance(target, selfIDAddress.getID());
		int fingerTableIndex = distance.bitLength() - 1;

		// init successor list index
		...
		} while (towardTargetComparator
				.compare(selfIDAddress, fingerTableEntry) <= 0);
