...
public final class RoutingTable {
	...
	private synchronized void initialize(int rowSize) {
		...
		for (int i = 0; i < rowSize; i++) {
			int digit = this.algorithm.getDigit(selfIDAddress.getID(), i);
	...
	public IDAddressPair merge(IDAddressPair entry) {
		int nMatchBits = ID.matchLengthFromMSB(selfIDAddress.getID(), entry.getID());
