...
/**
 * A routing driver which performs iterative lookup.
 *
		...
		if (true) {
			...
			try {
				if (config.getUseThreadPool()) {
					f = threadPool.submit((Callable<Boolean>)querier);
						...
						try {
							if (config.getUseThreadPool()) {
								...
								for (Querier q: forkedQueriers.values())
									fSet.add(threadPool.submit((Callable<Boolean>)q));
							...
							if (forkedRootCandidates[i] != null) {
								...
								if (!contact.equals(root)) {
									logger.log(Level.WARNING/*INFO*/,
											"The target of the terminating message is not the responsible node. adjusted from "
											+ contact.getAddress() + " to "
											+ root.getAddress() + ".");
