...
/**
 * A routing driver which performs recursive lookup.
 *
...
public final class RecursiveRoutingDriver extends AbstractRoutingDriver {
	...
	protected RecursiveRoutingDriver(RoutingServiceConfiguration conf,
			...
			ID selfID)
				throws IOException {
		super(conf, provider, receiver, threadPool, algoProvider, algoConf, selfID);
	...
	private RoutingResult[] route(
		...
		long sleepLimit = System.currentTimeMillis() + config.getRoutingTimeout();
...
//System.out.println("reply received: " + target[i].toString().substring(0, 4) + "..");
					...
					try {
						ClockJumpAwareSystem.wait(this.routeResultMsgTable, sleepPeriod);
					}
		...
		Set<ID> noReplyTarget = new HashSet<ID>();
		for (int i = 0; i < target.length; i++) {
			if (ret[i] == null) {
				noReplyTarget.add(target[i]);
			}
			else {

			}
		}
		if (!noReplyTarget.isEmpty()) {
			StringBuilder sb = new StringBuilder();
			for (ID id: noReplyTarget) {
				sb.append(" ").append(id);
			}

			logger.log(Level.WARNING, "Could not receive a REC_RESULT message in "
					+ config.getRoutingTimeout() + " msec for the target" + sb.toString());
		}
		...
		handler = new MessageHandler() {
			public Message process(Message msg) {
				...
				if (config.getUseThreadPool()) {
					threadPool.submit((Callable<Boolean>)f);
				}
				else {
...
//System.out.println("REC_REPLY message received: " + target[0].toString().substring(0, 4) + "..");
...
//System.out.flush();
		...
		if (terminate) {
			...
			for (int i = 0; i < target.length; i++) {
				if (!getSelfIDAddressPair().equals(rootCandidates[i][0].getIDAddressPair())) {
					adjustLastHop = true;
					terminates[i] = false;
					...
					logger.log(Level.WARNING/*INFO*/,
							"This node is not the responsible node for "	+ target[i]
							+ ". adjusted from "
							+ this.getSelfIDAddressPair().getAddress() + " to "
							+ rootCandidates[i][0].getAddress() + ".");
				}
				else {
...
//System.out.println("fork on " + getSelfIDAddressPair().getAddress());
					...
					if (config.getUseThreadPool()) {
						...
						for (Callable<Boolean> f: forkedForwarder)
							fSet.add(threadPool.submit(f));
					...
					do {
						try {
							...
							&& (blackListSet.contains(nextHops[i].getIDAddressPair())
									|| (oldMsgTag == Tag.ITE_ROUTE_JOIN.getNumber() && getSelfIDAddressPair().equals(nextHops[i]))));
...
//+ " for " + target[0].toString().substring(0, 4) + "..");
	...
	private final class Forwarder implements Callable<Boolean>, Runnable {
		private boolean adjustLastHop;
		private boolean terminate;
		private int msgTag; private IDAddressPair lastHop;
		private List<IDAddressPair>[] closestNodes; private RoutingContext[] routingContext;
		private ID[] target; private int numRootCandidates; private IDAddressPair initiator; private int ttl;
		private CallbackResultFilter filter; private int callbackTag; private Serializable[][] callbackArgs;
		private RoutingHop[] route;
		private IDAddressPair[] blackList;
