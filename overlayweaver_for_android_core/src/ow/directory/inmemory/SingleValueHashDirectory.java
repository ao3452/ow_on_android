...
public final class SingleValueHashDirectory<K,V> implements SingleValueDirectory<K,V> {
	...
	protected SingleValueHashDirectory(Class typeK, Class typeV, String workingDir, String dbName, String dbNameOld,long syncInterval) throws Exception {
		this.dbName = dbName;
...
/*		if (!loaded) {
			if (config.heapOverflowAction == HeapOverflowAction.LRU) {
				this.rawMap = new LinkedHashMap<K,V>() {
