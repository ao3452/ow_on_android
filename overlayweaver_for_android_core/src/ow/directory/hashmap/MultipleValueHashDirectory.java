...
public final class MultipleValueHashDirectory<K,V> implements MultiValueDirectory<K,V> {
	SingleValueDirectory<K,Map<V,V>> dir;
		// value of this directory should be Map, not Set
		// because it.remove() has to return a value.

	MultipleValueHashDirectory(Class typeK, Class typeV, String workingDir, String dbName, String dbNameOld, long syncInterval) throws Exception {
		this.dir = new SingleValueHashDirectory<K,Map<V,V>>(typeK, null /* Set<V>.class */, workingDir, dbName, dbNameOld, syncInterval);
	}
	...
	private class MultiValueHashDirectoryIterator implements Iterator<Entry<K,V>> {
		...
		MultiValueHashDirectoryIterator() {
			this.it0 = MultipleValueHashDirectory.this.dir.iterator();
			if (it0.hasNext()) {
				this.e0 = it0.next();
