...
public class TCPMessageReceiver implements MessageReceiver, Runnable {
	...
	private List<Relayhandler> rhandlerList = new ArrayList<Relayhandler>();
...
//				}
	...
	public void addrHandler(Relayhandler rhandler) {
		List<Relayhandler> newRelayHandlerList = new ArrayList<Relayhandler>();

		synchronized (this) {
			newRelayHandlerList.addAll(this.rhandlerList);	// copy
			newRelayHandlerList.add(rhandler);

			this.rhandlerList = newRelayHandlerList;	// substitute
		}

		if (rhandler instanceof ExtendedMessageHandler) {
			this.extMessageHandlerRegistered = true;
		}
	}
	...
	private class TCPMessageHandler implements Runnable {
		...
		public void run() {
			Thread.currentThread().setName("TCPMessageHandler: " + this.sock.socket().getInetAddress());
			...
			while (!Thread.interrupted()) {
				...
				/*if(msg.getTag()==Tag.Communicate.getNumber())
				{//Communicateのときのセッション継続-------------------------
					System.out.println("メッセージを受信");
					
					Socket socket = null;
					Socket dsocket = null;
					SocketChannel dsock = TCPMessageReceiver.this.CommunicateMessage(msg,sock);//---------ここで中継とかの処理
	...
	protected SocketChannel CommunicateMessage(Message msg, SocketChannel sock) {
		// call every handlers
		List<Relayhandler> currentHandlerList;
		synchronized (this) {
			currentHandlerList = rhandlerList;
		}
		SocketChannel dsock = null;
		for (Relayhandler rhandler: currentHandlerList) {
			try {
