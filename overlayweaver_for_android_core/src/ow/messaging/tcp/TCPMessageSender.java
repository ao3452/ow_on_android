...
public class TCPMessageSender implements MessageSender {
	...
	public Message sendAndReceive(MessagingAddress dest, final Message msg)
		...
		if (dest.equals(selfAddress)) {
			...
			if (this.receiver.extMessageHandlerRegistered) {
				...
				if (TCPMessageSender.this.receiver.config.getUseThreadPool()) {
					this.receiver.provider.getThreadPool().submit(r);
				}
				else {
		...
		while (true) {
			...
			//Timeoutno値はMesageconfiguration.javaで管理
	...
	public SocketChannel sendAndCommunicate(MessagingAddress dest, Message msg) throws IOException {//メッセージ交換
		this.adjustLoopbackAddress((InetMessagingAddress)dest);	
	
		boolean success = false;//-----送信が成功したかどうか
		SocketAddress sockAddr = ((InetMessagingAddress)dest).getInetSocketAddress();
		SocketChannel sock = null;
		...
		while (true) {
			...
			try {
				this.sendJoin(sock, dest, msg);// <----------------メッセージ交換
				success = true;//メッセージ交換が出来た
				break;
			}
			catch (ClosedChannelException e) {
				// sock is stale. retry once.
	...
	private void sendJoin(SocketChannel sock, MessagingAddress dest, Message msg)  throws IOException {
		logger.log(Level.INFO, "send(" + dest + ", " + Tag.getNameByNumber(msg.getTag()) + ")");
	...
	public SocketChannel sendAndSockGet(MessagingAddress dest, Message msg) throws IOException {
		...
		while (true) {
			try {
				...
				logger.log(Level.INFO, "Failed to sockget: " + dest);
				// rethrow Exception and dispose socket
				throw e;
			}
			...
			try {
				this.send(sock,dest, msg);// <----------------メッセージ交換
				success = true;//メッセージ交換が出来た
				break;
			}
			catch (ClosedChannelException e) {
